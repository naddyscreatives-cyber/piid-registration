  ...
  return (
    <div className="w-full animate-pop-in rounded-3xl bg-white p-7 text-center shadow-card sm:p-10">
      ...
      <p className="ui-body mx-auto mt-4 max-w-sm">
        {result.emailSent
          ? 'Your QR pass is on its way to your inbox. Save it below too — venue signal can be unreliable on the day.'
          : 'Here is your QR pass. Save it to your phone and present it for check-in at each showroom.'}
      </p>

      {/* ===== SECTION: QR PASS (large + downloadable) ===== */}
      {result.qrDataUrl ? (
        <div className="mt-7">
          <div className="mx-auto w-full max-w-[20rem] rounded-3xl bg-white p-4 shadow-card ring-2 ring-slate-900 sm:p-5">
            ...
            />
            <p className="mt-3 text-[11px] font-bold uppercase tracking-[0.24em] text-slate-900">
              {result.firstName} &middot; QR Pass
            </p>
          </div>

          <a
            href={result.qrDataUrl}
            download={`${EVENT_NAME.replace(/\s+/g, '-')}-QR-Pass-${result.firstName}.png`}
            className="ui-btn-primary mx-auto mt-6 flex max-w-[20rem] items-center justify-center gap-2.5"
          >
            <UiDownloadIcon />
            Download QR Pass
          </a>
          <p className="mt-3 text-xs leading-relaxed text-slate-400">
            Or take a screenshot &mdash; this code is your pass for every showroom.
          </p>
        </div>
      ) : null}
...
/**
 * UiDownloadIcon — arrow-into-tray glyph for the Download QR Pass button.
 * @returns {JSX.Element}
 */
function UiDownloadIcon() {
  return (
