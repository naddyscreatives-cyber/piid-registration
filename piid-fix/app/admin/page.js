  ...
  const api_loadSession = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 401) {
        // We HAD a valid browser session and the server still could not verify it.
        // That is a server configuration problem (usually a missing or mismatched
        // anon key), not an expired login — so say so instead of bouncing back to
        // the sign-in screen, which looks identical to a rejected password.
        setProblem(
          'You signed in, but the server could not verify the session. This is almost always a configuration problem rather than a wrong password — open /api/health on this site to see which value is missing or mismatched.'
        );
        setPhase('blocked');
        return;
      }
