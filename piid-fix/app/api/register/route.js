...
export async function POST(request) {
  ...
  let qrToken;
  try {
    qrToken = qr_createToken(registrantId);
  } catch (err) {
    // Almost always a missing QR_SECRET. Without this guard the throw escaped as
    // a bare 500 and the form showed the same generic banner as every other
    // failure, hiding a one-line configuration fix.
    log_error('REGISTER', 'could not sign QR token — check QR_SECRET', err);
    return NextResponse.json(
      { ok: false, message: 'Something went wrong — please try again.' },
      { status: 503 }
    );
