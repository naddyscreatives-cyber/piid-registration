...
import { env_require, env_requireAny } from './env';
import { log_line, log_shortId } from './logger';

/** Either spelling of the project URL is accepted (see env_requireAny). */
const URL_NAMES = ['SUPABASE_URL', 'NEXT_PUBLIC_SUPABASE_URL'];
/** Either spelling of the anon/publishable key is accepted. */
const ANON_NAMES = ['SUPABASE_ANON_KEY', 'NEXT_PUBLIC_SUPABASE_ANON_KEY'];
...
export function db_clientForAccessToken(accessToken) {
  return createClient(env_requireAny(URL_NAMES), env_requireAny(ANON_NAMES), {
    auth: { persistSession: false, autoRefreshToken: false },
